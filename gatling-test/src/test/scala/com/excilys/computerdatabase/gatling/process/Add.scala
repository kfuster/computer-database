...
object Add {
  ...
  val add = exec(http("Add: Add page")
    .get(config.getString("application.urls.addPage")).digestAuth(config.getString("application.urls.form.authenticate.username"), config.getString("application.urls.form.authenticate.password")).check(status.is(200))
    ...
    .exec(http("Add: Add post")
      .post(config.getString("application.urls.addPost").get).digestAuth(config.getString("application.urls.form.authenticate.username"), config.getString("application.urls.form.authenticate.password"))
