...
object Delete {
  ...
  val delete = exec(http("Delete: Search for delete")
    ...
    .check(
      status.is(200),
      css("#computerId", "value").saveAs("computerId")
    ))
    .pause(3, 10)
    .exec(http("Delete: Delete post")
      .post(config.getString("application.urls.deletePost").get).digestAuth(config.getString("application.urls.form.authenticate.username"), config.getString("application.urls.form.authenticate.password"))
